...
class ComplexData(DATA.Data):
   
    def __inc__(self, key, value, *args, **kwargs):
       
        if key == 'target_edge_index':
            return self.target_x.size(0)
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xdt, xt, y, smile_graph):
        assert (len(xd) == len(xt) and len(xt) == len(y) == len(xdt)), "The lists must be the same length!"

        # Pad the target SMILES sequences (for generation task)
        ...
        for i in range(data_len):
            if i % 1000 == 0:
                print('Preparing data: {}/{}'.format(i, data_len))
            
            smiles = xd[i]
            target_graph = xt[i] 
            labels = y[i]
            tok_smi = smi[i].tolist() 
            ...
            GCNData = ComplexData(
                ...
                target_x = target_graph.x, 
                target_edge_index = target_graph.edge_index,
                target_edge_attr = target_graph.edge_attr
            )
            ...
            GCNData.c_size = torch.LongTensor([c_size]) 
            GCNData.target_size = torch.LongTensor([target_graph.x.size(0)]) 
