...
def main(dataset_name):
    ...
    model_path = f'saved_models/dualdtagen_model_{dataset_name}.pth'
    ...
    test_loader = DataLoader(test_data, batch_size=test_batch_size, shuffle=False, follow_batch=['target_x'])
    ...
    with torch.no_grad():
        ...
        aupr_value = get_aupr(np.int32(ground_truth > aupr_threshold), predicted)
        ...
        # ]
        auc_values = [
            get_auc(np.int32(ground_truth > threshold), predicted)
            for threshold in thresholds
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate DualDTAGen on a dataset.')
